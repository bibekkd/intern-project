    ...
    while (retryCount < maxRetries) {
      try {
        const systemPrompt = `You are a Gen-Z tutor who explains complex topics concisely for a ${userContext.age} year old.
          ${userContext.isFollowUp ? `
          Previous Context:
          ${userContext.previousContext}
          
          Now provide a follow-up explanation that builds upon this context.
          ` : 'Provide a fresh explanation of the topic.'}
          ...
          <paragraph 1>
          ${userContext.isFollowUp ? '(Connect to previous context and expand)' : '(Basic definition)'}

          <paragraph 2>
          ${userContext.isFollowUp ? '(Provide new insights building on previous knowledge)' : '(Key details)'}

          <paragraph 3>
          ${userContext.isFollowUp ? '(Show advanced applications or deeper connections)' : '(Direct applications and facts)'}
          ...
          - ADAPT CONTENT FOR ${userContext.age} YEAR OLD:
            * Match complexity to age level
            ${userContext.isFollowUp ? '* Build upon previously explained concepts' : '* Start from basics'}
          ...
          - Focus on deeper insights and connections
          - Show how new information connects to previous context
          - Maintain coherent narrative flow from previous explanation
          ` : ''}
          ...
          - Related questions must be:
            ...
            * Make users think "Wow, I never thought about that!"
            ${userContext.isFollowUp ? '* Build upon previous questions and topics' : ''}
          ...
          - Related topics must be:
            ...
            * Brief, clear explanation of importance
            ${userContext.isFollowUp ? '* Connect to previously discussed topics' : ''}
        ...
        const userPrompt = `${userContext.isFollowUp ? `
        Based on the previous context about ${userContext.previousQuery}, explain "${query}" as a follow-up topic.
        ` : `Explain "${query}"`} in three very concise paragraphs for a ${userContext.age} year old in genz style:

        1. ${userContext.isFollowUp ? 'Connect to previous knowledge and expand (15-20 words)' : 'Basic definition (15-20 words)'}
        2. ${userContext.isFollowUp ? 'New insights and connections (15-20 words)' : 'Key details (15-20 words)'}
        3. ${userContext.isFollowUp ? 'Advanced applications or deeper implications (15-20 words)' : 'Direct applications and facts (15-20 words)'}

        Then provide EXACTLY:
        - 5 related topics that ${userContext.isFollowUp ? 'build upon previous knowledge and' : ''} help understand ${query} better (age-appropriate)
        - 5 mind-blowing questions (8-12 words each) that spark curiosity ${userContext.isFollowUp ? 'and connect to previous context' : ''}
